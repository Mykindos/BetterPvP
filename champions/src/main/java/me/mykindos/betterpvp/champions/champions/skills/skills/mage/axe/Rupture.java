...
public class Rupture extends Skill implements Listener, InteractSkill, CooldownSkill, AreaOfEffectSkill, DebuffSkill, DamageSkill {
    ...
    // Create a vector for the initial direction based on the player's look (this direction will be fixed unless right-click is held)
    Vector[] vector = {player.getLocation().getDirection().normalize().multiply(0.3D)};
    vector[0].setY(0);  // Ensure the movement is horizontal
    
    // The initial location where the path starts
    Location loc = player.getLocation().subtract(0.0D, 1.0D, 0.0D).add(vector[0]);
    ...
    final BukkitTask runnable = new BukkitRunnable() {
        ...
        public void run() {
            ...
            for (int i = 0; i < 3; i++) {
                loc.add(vector[0]);  // Update the position of the armor stand based on the vector
                ...
                // Handle entity effects and damage
