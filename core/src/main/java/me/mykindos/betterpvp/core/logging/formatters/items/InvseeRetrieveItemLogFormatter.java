...
@WithReflection
public class InvseeRetrieveItemLogFormatter implements ILogFormatter {

    @Override
    public String getAction() {
        return "ITEM_INVSEE_RETRIEVE";
    }
    ...
    public Description getDescription(CachedLog cachedLog, LogRepository logRepository, Windowed previous) {
        ...
        List<? extends LogRepositoryButton> buttons = List.of(
                ...
                new PlayerItemButton(context.get(LogContext.CLIENT_NAME), context.get(LogContext.CLIENT), "Taker", JavaPlugin.getPlugin(Core.class), logRepository, previous),
                new PlayerItemButton(context.get(LogContext.TARGET_CLIENT_NAME), context.get(LogContext.TARGET_CLIENT), "Target", JavaPlugin.getPlugin(Core.class), logRepository, previous)
        );
