...
public abstract class ArmorItem extends BaseItem implements Reloadable {
    ...
    public void reload() {
        ...
        final int health = config.getConfig("health.base", 1, Integer.class);
        final int healthMin = config.getConfig("health.min", 0, Integer.class);
        final int healthMax = config.getConfig("health.max", 2, Integer.class);
