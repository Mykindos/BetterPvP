...
public class EnergyOre extends CustomOre implements Listener {
    ...
    public @NotNull BlockData getType() {
        ...
        return items.get(random.nextInt(items.size())).getMaterial().createBlockData();
    }
    ...
    public @NotNull ItemStack @NotNull [] generateDrops(@NotNull FieldsBlock fieldsBlock) {
        ...
        int amount = switch (energyItem) {
            case SMALL_CRYSTAL -> minEnergy + random.nextInt(range / 3);
            case LARGE_CRYSTAL -> minEnergy + range / 3 + random.nextInt(range / 3);
            case GIANT_CRYSTAL -> minEnergy + 2 * range / 3 + random.nextInt(range / 3);
            default -> 0;
        };
