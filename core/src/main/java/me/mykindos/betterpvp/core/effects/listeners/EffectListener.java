...
public class EffectListener implements Listener {
    ...
    public void onReceiveEffect(EffectReceiveEvent event) {
        ...
        if (effectsOptional.isPresent()) {
            ...
            if (effect.getEffectType() == EffectType.STRENGTH) {
                ...
                target.addPotionEffect(new PotionEffect(PotionEffectType.INVISIBILITY, (int)((effect.getRawLength() / 1000d) * 20), 0, true, true));
    ...
    public void playStepSoundsForConfusedPlayers() {
        Bukkit.getOnlinePlayers().forEach(player -> {
            if (effectManager.hasEffect(player, EffectType.CONFUSION)) {
