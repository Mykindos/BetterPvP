...
@WithReflection
public class SellItemLogFormatter implements ILogFormatter {
    ...
    public Component formatLog(HashMap<String, String> context) {
        return Translations.component("core.log.sell-item.1",
    ...
    public Description getDescription(CachedLog cachedLog, LogRepository logRepository, Windowed previous) {
        ...
        List<Component> lore = List.of(
                ...
                UtilMessage.DIVIDER,
                Component.text(context.get(LogContext.CLIENT_NAME), NamedTextColor.YELLOW)
                        .append(Translations.component("core.log.sell-item.2").color(NamedTextColor.GRAY)),
                Component.text(context.get(LogContext.ITEM_NAME), NamedTextColor.GREEN),
                UtilMessage.deserialize("(<light_purple>%s</light_purple>)",
                        context.get(LogContext.ITEM)),
                Translations.component("core.log.sell-item.3",
        ...
        List<? extends PreviousableButton> buttons = List.of(
                ...
                new PlayerItemButton(context.get(LogContext.CLIENT_NAME), context.get(LogContext.CLIENT), "Seller", JavaPlugin.getPlugin(Core.class), logRepository, previous),
                new LocationButton(UtilWorld.stringToLocation(context.get(LogContext.LOCATION)), true, previous)
        );

        ItemProvider itemProvider = ItemView.builder()
                .displayName(Translations.component("core.log.sell-item.4"))
