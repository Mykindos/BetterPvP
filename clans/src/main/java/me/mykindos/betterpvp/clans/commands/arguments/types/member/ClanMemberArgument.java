...
public class ClanMemberArgument extends PlayerNameArgumentType {
    ...
    public static final DynamicCommandExceptionType MEMBER_CLAN_CANNOT_ACTION_SELF = new DynamicCommandExceptionType(
            (action) -> new LiteralMessage("You cannot " + action + " yourself")
    );

    public static final DynamicCommandExceptionType MEMBER_CANNOT_PROMOTE_MEMBER_RANK = new DynamicCommandExceptionType(
            (targetName) -> new LiteralMessage("Rank insufficient to promote " + targetName)
    );
    ...
    public <S> @NotNull String convert(@NotNull String nativeType, @NotNull S source) throws CommandSyntaxException {
        ...
        clanManager.canPromoteThrow(executorMember, target);
        return name;
    }
