...
public class ScytheData {
    ...
    public Soul getTargetSoul() {
        return scythe.getSoulHarvestAbility().getSouls().get(targetSoul);
    }
    ...
    public double getSoulCount() {
        return chargeData.getCharge() * scythe.getSoulHarvestAbility().getMaxSouls();
    }

    public void setSoulCount(double count) {
        chargeData.setCharge((float) (count / scythe.getSoulHarvestAbility().getMaxSouls()));
    }
    ...
    public boolean attemptHarvest() {
        if (isHarvesting()) {
            if (System.currentTimeMillis() - lastHarvestStart >= scythe.getSoulHarvestAbility().getSoulHarvestSeconds() * 1000) {
                gainSoul(getTargetSoul().getCount());
                scythe.getSoulHarvestAbility().getSouls().remove(targetSoul); // Remove the soul from the world
                stopHarvesting();
                return true;
            }
        }
    ...
    public boolean gainSoul(double count) {
        nextSoulExpiry = System.currentTimeMillis() + (long) (scythe.getSoulHarvestAbility().getSoulExpirySeconds() * 1000);
        if (canHarvest()) {
            setSoulCount(Math.min(scythe.getSoulHarvestAbility().getMaxSouls(), getSoulCount() + count));
            return true;
        }
        return false;
    }
    ...
    public boolean canHarvest() {
        return getSoulCount() < scythe.getSoulHarvestAbility().getMaxSouls();
    }
    ...
    public void playHarvest(@Nullable Soul soul) {
        ...
        // Sounds
        final float pitch = 2 * ((float) getSoulCount() / scythe.getSoulHarvestAbility().getMaxSouls());
