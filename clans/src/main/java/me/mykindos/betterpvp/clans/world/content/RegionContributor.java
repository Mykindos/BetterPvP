 ...
 * puts a build into the world at runtime — a ship pasted at a berth brings its helm, its crew NPC and its hull bounds
...
public interface RegionContributor {
     ...
     * @param authored what the world's own Mapper file declares — a contributor is usually triggered by a marker in
     *                 here (a berth saying which structure belongs at it), so it is handed over rather than re-read
     * @return the regions to add to this world's index for this load; empty if this contributor has nothing here
     */
