...
SELECT L1.id as ID, L1.Time, L1.type, C1.Player1, C2.Clan1, CN1.Name as Clan1Name, C3.Player2, C4.Clan2, CN2.Name as Clan2Name FROM logmeta L1
    ...
    LEFT JOIN clans_names as CN1 on CN1.id = C2.Clan1
    LEFT JOIN clans_names as CN2 on CN2.id = C4.Clan2
    WHERE L1.type LIKE 'CLAN_%';

DROP PROCEDURE IF EXISTS GetClanLogsByClan;
CREATE PROCEDURE GetClanLogsByClan(ClanID varchar(36))
BEGIN
    SELECT DISTINCT CL.Time, CL.type, CL.Player1, CL.Clan1, CL.Clan1Name, CL.Player2, CL.Clan2, CL.Clan2Name
    FROM clanlogs CL
...
BEGIN
    SELECT CL.Clan1 as ClanID, Cl.Clan1Name as ClanName FROM clanlogs CL
...
BEGIN
    SELECT DISTINCT CL.Clan1 as Clan, CL.Clan1Name as ClanName FROM clanlogs CL
