...
public class BrigadierNeutralCommand extends BrigadierClanSubCommand {
    ...
    public LiteralArgumentBuilder<CommandSourceStack> define() {
        return Commands.literal(getName())
                .then(Commands.argument("Ally Or Enemy Clan", BPvPClansArgumentTypes.allyOrEnemyClan())
                        ...
                        //TODO figure out why this results in no suggestions when there are valid ones
                        //TODO is the last parsed argument overriding?
                )/*.then(Commands.argument("Ally Clan", BPvPClansArgumentTypes.allyClan())
                        .executes(context -> {
                            Clan target = context.getArgument("Ally Clan", Clan.class);
                            if (!(context.getSource().getExecutor() instanceof Player player)) return Command.SINGLE_SUCCESS;
    ...
    private void doNeutral(Player originPlayer, Clan origin, Clan target) throws CommandSyntaxException {
        if (!target.isAllied(origin) || target.isEnemy(origin)) throw ClanArgument.CLAN_NOT_ALLY_OR_ENEMY_OF_CLAN.create(origin.getName(), target.getName());
