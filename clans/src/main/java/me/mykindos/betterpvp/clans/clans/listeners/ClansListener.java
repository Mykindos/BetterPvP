...
@BPvPListener
public class ClansListener extends ClanListener {
    ...
    @Inject
    public ClansListener(ClanManager clanManager, GamerManager gamerManager, InviteHandler inviteHandler, Clans clans) {
    ...
    public void onClanEvent(ClanEvent event) {
        if (event.isGlobalScoreboardUpdate()) {
            Bukkit.getOnlinePlayers().forEach(player -> UtilServer.runTaskLater(clans,
                    () -> UtilServer.callEvent(new ScoreboardUpdateEvent(event.getPlayer())), 5));
        } else {
    ...
    public void onClanCreate(ClanCreateEvent event) {
        ...
        clanManager.addObject(clan.getName(), clan);
