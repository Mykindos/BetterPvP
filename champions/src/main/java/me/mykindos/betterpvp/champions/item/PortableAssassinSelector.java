...
@ItemKey("champions:portable_assassin_selector")
@Singleton
public class PortableAssassinSelector extends BaseItem {

    private transient boolean registered;

    @Inject
    public PortableAssassinSelector() {
        super("Portable Assassin Selector",
                Item.builder(Material.ORANGE_HARNESS).maxStackSize(1).build(),
                ItemGroup.CONSUMABLE,
                ItemRarity.UNCOMMON);
        final PortableClassAbility portableClassAbility = new PortableClassAbility(Role.ASSASSIN);
        portableClassAbility.setConsumesItem(true);
        addSerializableComponent(new ArmorStorageComponent(Role.ASSASSIN, false));
    ...
    private void registerRecipe(CraftingRecipeRegistry registry, ItemFactory itemFactory, Rope rope) {
        ...
        registered = true;
        final BaseItem ore = itemFactory.getFallbackItem(Material.LEATHER);
        ...
        registry.registerRecipe(new NamespacedKey("champions", "portable_assassin_selector"), builder.build());
    }
}
