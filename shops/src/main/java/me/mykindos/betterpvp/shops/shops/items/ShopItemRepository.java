    ...
    public HashMap<String, List<IShopItem>> getAllShopItems() {
        ...
        try (CachedRowSet result = database.executeQuery(new Statement(query))) {
            while (result.next()) {
                ...
                try (CachedRowSet dynamicPricingResult = database.executeQuery(new Statement(dynamicPricingQuery, new IntegerStatementValue(id)))) {
                ...
                try(CachedRowSet itemFlagResult = database.executeQuery(new Statement(itemFlagQuery, new IntegerStatementValue(id)))) {
            ...
            log.error("Error loading shop items", ex);
        }
