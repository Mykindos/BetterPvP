...
public class PlayerList {
    ...
    private final List<DisplayObject<Component>> headers = new ArrayList<>();
    private final List<DisplayObject<Component>> footers = new ArrayList<>();

    // Use a lock to synchronize access to the components PriorityQueue
    private final Object lock = new Object();

    public List<DisplayObject<Component>> getHeader() {
        synchronized (lock) {
            return headers;
        }
    }

    public List<DisplayObject<Component>> getFooter() {
        synchronized (lock) {
            return footers;
        }
    }

    public void add(PlayerListType type, DisplayObject<Component> component) {
        synchronized (lock) {
            List<DisplayObject<Component>> components = type == PlayerListType.HEADER ? headers : footers;
    ...
    public void remove(PlayerListType type, DisplayObject<Component> component) {
        synchronized (lock) {
            List<DisplayObject<Component>> components = type == PlayerListType.HEADER ? headers : footers;
    ...
    private Component getComponent(PlayerListType type, Gamer gamer) {
        synchronized (lock) {
            ...
            final Iterator<DisplayObject<Component>> iterator = components.iterator();
            Component advComponent = null;

            do {
                DisplayObject<Component> display = iterator.next();
    ...
    private void cleanUp() {
        synchronized (lock) {
            ...
            headers.removeIf(DisplayObject::isInvalid);
            footers.removeIf(DisplayObject::isInvalid);
        }
    }
}