...
/**
 * Where a structure's build lands for a given version and position. Footprints are kept once worked out, since the fit
...
public class StructureShapes {
    ...
    public @NotNull Optional<SchematicPlacement> placementOf(@NotNull World world, @NotNull PlacedStructure structure) {
        return placementOf(world, structure.getType(), structure.getVersion(), structure.getPosition());
    }

    public @NotNull Optional<SchematicPlacement> placementOf(@NotNull World world, @NotNull String type, int version,
                                                             @NotNull StructurePosition position) {
        return catalogue.find(type)
                .flatMap(found -> schematics.load(found.version(version).getSchematic()))
                .map(schematic -> SchematicPlacement.of(schematic, position.toLocation(world), position.getQuarterTurns()));
    }

    public @NotNull Optional<Footprint> footprintOf(@NotNull World world, @NotNull String type, int version,
                                                    @NotNull StructurePosition position) {
        final String key = type + ":" + version + ":" + position.getX() + ":" + position.getY() + ":" + position.getZ()
                + ":" + position.getQuarterTurns();
        final Footprint known = footprints.get(key);
        if (known != null) {
            return Optional.of(known);
        }
        return placementOf(world, type, version, position).map(placement -> {
